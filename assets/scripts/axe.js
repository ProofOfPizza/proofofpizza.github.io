...
const activateSymbol = (symbol, node) => {
  ...
  switch (symbol) {
    case "<":
      targetElements = ancestorImmediate(node);
      break;
    case "^":
      targetElements = ancestorAll(node);
      break;
    case "%":
      targetElements = siblingImmediate(node);
      break;
    case "|":
      targetElements = siblingAll(node);
      break;
    case "?":
      targetElements = siblingImmediatePrevious(node);
      break;
    case "!":
      targetElements = siblingAllPrevious(node);
      break;
  }
...
const activateQuery = (querySelector, segmentName) => {
  ...
  while (querySelector.length > querySegment + 2) {
    ...
    var queryPattern = querySelectorFragment.replace(
      /([^\]]+\])([^\:]+)(\:[^\s]+)(.*)/,
      "$1$2$4"
    );
    ...
    for (let a = 1; a < queryNewSegment; a++) {
      queryCurrentAttribute += querySelector[a];
    }
    var queryNextAttribute = "";
    for (let a = 1; a < queryNewSegment + 1; a++) {
      queryNextAttribute += querySelector[a];
    }

    for (let j = 0; j < queryNodes.length; j++) {
      var queryNode = queryNodes[j];

      var queryNeedle = querySelector[queryNewSegment]
        .substring(3)
        .replace(/\:[^\s]+/g, "");
      ...
      queryTargetElements.forEach(function (queryTargetElement) {
        if (
          queryTargetElement[nodeProperties(queryNeedle).label] ===
          nodeProperties(queryNeedle).name
        ) {
          queryTargetElement.setAttribute(
            "data-" + segmentName + "-segment-" + (querySegment + 1),
            ""
          );
        }
      });
    ...
    querySelectorFragment =
      "[data-" + segmentName + "-segment-" + (querySegment + 1) + "]";
    querySelectorFragment += querySelector[queryNewSegment + 1]
      ? querySelector[queryNewSegment + 1]
      : "";
  ...
  if (querySelector.length <= querySegment + 2) {
    if (
      segmentName.substr(-7, 7) === "-target" &&
      querySelector.length % 2 === 0
    ) {
      querySelectorFragment = querySelectorFragment.replace(
        /([^\]]+\]).*/,
        "$1"
      );
    }
...
const pseudoHover = (axeRule) => {
  if (axeRule.axeSelector.length % 2 > 0) {
    var dataAttribute =
      "axe-" + (axeRule.axeIndex * 100 + (axeRule.axeSelector.length - 2));
  } else {
    ...
    var dataAttribute =
      "axe-" + (axeRule.axeIndex * 100 + (axeRule.axeSelector.length - 2) - 1);
  }
  ...
  for (let b = 1; b < axeRule.axeSelector.length; b++) {
    if (bladeCompleted !== true) {
      ...
      if (axeRule.axeSelector[b].match(/\:/)) {
        bladeSelector[bladeSelector.length - 1] = bladeSelector[
          bladeSelector.length - 1
        ].replace(/([^\:]+):hover(.*)/, "$1$2");
        bladeCompleted = true;
      }
  ...
  var bladeSelectorFragment = activateQuery(
    bladeSelector,
    "axe-" + axeRule.axeIndex + "-blade"
  );
  ...
  axeBlades.forEach(function (axeBlade, bladeIndex) {
    ...
    axeBlade.setAttribute(
      "data-axe-" + axeRule.axeIndex + "-blade-" + bladeIndex,
      bladeSelector[0]
    );
    ...
    var targetSelectorFragment = activateQuery(
      targetSelector,
      "axe-" + axeRule.axeIndex + "-target"
    );

    var axeTargets = document.querySelectorAll(targetSelectorFragment);

    axeTargets.forEach(function (axeTarget) {
      axeTarget.setAttribute(
        "data-axe-" + axeRule.axeIndex + "-target-" + bladeIndex,
        targetSelector[0]
      );
    });

    for (let t = 0; t < targetSelector.length; t++) {
      var axeTargetSegments = document.querySelectorAll(
        "[data-axe-" + axeRule.axeIndex + "-target-segment-" + t + "]"
      );
      for (let s = 0; s < axeTargetSegments.length; s++) {
        axeTargetSegments[s].removeAttribute(
          "data-axe-" + axeRule.axeIndex + "-target-segment-" + t + ""
        );
      }
    }

    var bladeTargets = document.querySelectorAll(
      "[data-axe-" + axeRule.axeIndex + "-target-" + bladeIndex + "]"
    );

    axeBlade.addEventListener(
      "mouseover",
      function () {
        bladeTargets.forEach(function (bladeTarget) {
          bladeTarget.dataset[dataAttribute] = bladeTarget.dataset[
            dataAttribute
          ].replace("&", ":");
        });
      },
  ...
  for (let b = 0; b < bladeSelector.length; b++) {
    var axeBladeSegments = document.querySelectorAll(
      "[data-axe-" + axeRule.axeIndex + "-blade-segment-" + b + "]"
    );
    for (let s = 0; s < axeBladeSegments.length; s++) {
      axeBladeSegments[s].removeAttribute(
        "data-axe-" + axeRule.axeIndex + "-blade-segment-" + b + ""
      );
    }
  }
  ...
  for (let as = 0; as < axeRule.axeSelector.length * 2; as++) {
    var axeBlades = document.querySelectorAll(
      "[data-axe-" + axeRule.axeIndex + "-blade-" + as + "]"
    );
    var axeTargets = document.querySelectorAll(
      "[data-axe-" + axeRule.axeIndex + "-target-" + as + "]"
    );

    for (let ab = 0; ab < axeBlades.length; ab++) {
      axeBlades[ab].removeAttribute(
        "data-axe-" + axeRule.axeIndex + "-blade-" + as
      );
    }

    for (let at = 0; at < axeTargets.length; at++) {
      axeTargets[at].removeAttribute(
        "data-axe-" + axeRule.axeIndex + "-target-" + as
      );
    }
  }
...
const initialiseStylesheets = () => {
  ...
  for (let i = 0; i < stylesheets.length; i++) {
    ...
    for (let j = 0; j < stylesheet.length; j++) {
      if (!stylesheet[j].match(/(^$|^\s+$)/)) {
        if (
          !stylesheet[j].match(/\:/) ||
          stylesheet[j].match(
            /\:\:|\:nth|\:first|\:last|\:link|\:visited|\:hover|\:active|\:focus|\:target/
          )
        ) {
          ...
          if (stylesheet[j].match(/\,/g)) {
            ...
            for (let r = 0; r < ruleCount; r++) {
              if (ruleGroup[r].match(/[\<\^\?\!\%\|]/) === null) {
                var rulePosition =
                  ruleIndex - 1 > document.styleSheets[0].cssRules.length
        ...
        stylesheetRules[stylesheetRules.length] = stylesheet[j]
          .replace(/([\s]*:[\s]*)/, ":")
          .trim();
      }
    }
    ...
    for (let j = 0; j < stylesheetRules.length; j++) {
      if (
        typeof stylesheetRules[j] === "number" &&
        stylesheetRules[j + 1].match(/(\<|\^|\?|\!|\%|\|)/)
      ) {
        ...
        while (typeof stylesheetRules[k] === "string") {
          var property = stylesheetRules[k].substring(
            0,
            stylesheetRules[k].indexOf(":")
          );
          var value = stylesheetRules[k].substring(
            stylesheetRules[k].indexOf(":") + 1
          );
  ...
  axe.axeRules.forEach(function (axeRule) {
    axeStyle(axeRule);
  });
...
const getAxeStyles = () => {
  ...
  for (let i = styleSheetPaths.length - 1; i > -1; i--) {
    ...
    axeStyleSheet.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        axeStylesElement.textContent =
          this.responseText + axeStylesElement.textContent;
      }
    };
  ...
  if (typeof axeStyleSheet != "undefined") {
    axeStyleSheet.addEventListener(
      "load",
      function () {
    ...
    axeStylesElement.textContent = document.head.getElementsByTagName(
      "style"
    )[0].textContent;
    initialiseStylesheets();
  }
...
const axeStyle = (axeRule) => {
  ...
  while (axeRule.axeSelector.length > segment + 2) {
    ...
    var pattern = selectorFragment.replace(
      /([^\]]+\])([^\:]+)(\:[^\s]+)(.*)/,
      "$1$2$4"
    );
    ...
    for (let a = 1; a < newSegment; a++) {
      currentAttribute += axeRule.axeSelector[a];
    }
    var nextAttribute = "";
    for (let a = 1; a < newSegment + 1; a++) {
      nextAttribute += axeRule.axeSelector[a];
    }
    ...
    for (let j = 0; j < nodes.length; j++) {
      ...
      if (axeRule.axeSelector[segment + 1].match(/[\:]/)) {
        node.setAttribute(
          "data-axe-" + axeRule.axeIndex + ("0" + segment).slice(-2),
          currentAttribute.replace(":", "&")
        );
      }

      var needle = axeRule.axeSelector[newSegment]
        .substring(3)
        .replace(/\:[^\s]+/g, "");
        ...
        if (nodeProperties(needle).hasOwnProperty("qualifierClass")) {
          if (
            targetElement.classList.contains(
              nodeProperties(needle).qualifierClass
            )
          ) {
            completeLabel +=
              "." + nodeProperties(needle).qualifierClass.toUpperCase();
          }
        }

        if (completeLabel === nodeProperties(needle).name) {
          targetElement.setAttribute(
            "data-axe-" + axeRule.axeIndex + ("0" + (segment + 1)).slice(-2),
            nextAttribute.replace(":", "&")
          );
    ...
    selectorFragment =
      ...
      nextAttribute +
      '"]';
    selectorFragment += axeRule.axeSelector[newSegment + 1]
      ? axeRule.axeSelector[newSegment + 1]
      : "";
    segment = newSegment;
  }

  for (
    let a = axeRule.axeIndex * 100;
    a < axeRule.axeIndex * 100 + (segment - 1);
    a++
  ) {
  ...
  document.styleSheets[0].insertRule(
    selectorFragment + "{" + styleString(axeRule.axeStyles) + "}",
    axeRule.axeIndex
  );

  if (
    axeRule.axeSelector[0].match(
      /\:hover|\:click|\:rightclick|\:doubleclick|\:keypress|\:mousemove|\:resize|\:scroll|\:blur|\:focus|\:change|\:invalid|\:reset|\:search|\:select|\:submit/
    )
  ) {
    var pseudoElement = axeRule.axeSelector[0].replace(
      /[^\:]+(\:[^\s]+).*/,
      "$1"
    );
    ...
    switch (pseudoElement) {
      ...
      case ":click":
        ...
        break;
      case ":rightclick":
        pseudoRightClick(axeRule);
        break;
      case ":doubleclick":
        pseudoDoubleClick(axeRule);
        break;
      case ":keypress":
        pseudoKeyPress(axeRule);
        break;
      case ":mousemove":
        pseudoMouseMove(axeRule);
        break;
      case ":resize":
        pseudoResize(axeRule);
        break;
      case ":scroll":
        pseudoScroll(axeRule);
        break;
      case ":blur":
        pseudoBlur(axeRule);
        break;
      case ":focus":
        ...
        break;
      case ":change":
        pseudoChange(axeRule);
        break;
      case ":invalid":
        pseudoInvalid(axeRule);
        break;
      case ":reset":
        pseudoReset(axeRule);
        break;
      case ":search":
        pseudoSearch(axeRule);
        break;
      case ":select":
        pseudoSelect(axeRule);
        break;
      case ":submit":
        pseudoSubmit(axeRule);
        break;
    }
  }
};
...
document
  ...
  .forEach(function (styleSheetLink, i) {
...
const convertQuery = (query) => {
  ...
  bladeCoverArray[bladeStart] = bladeCoverArray[bladeStart].replace(
    ":hover",
    ""
  );
  ...
  for (
    let bladeCover = bladeCoverArray.length - 1;
    bladeCover > -1;
    bladeCover--
  ) {
